...
layouts['2rows'] = function(windows)
	local winCount = #windows
	...
	firstRow = winCount // 2
	secondRow = winCount - firstRow

	for index, win in pairs(windows) do
		local frame = win:screen():frame()

		frame.h = frame.h /2 - 10 -- 10 points to account for windows chrome
		if index <= (firstRow) then
			frame.w = frame.w / firstRow
			frame.y = 0
			frame.x = frame.x + (index - 1) * frame.w
		else
			frame.w = frame.w / secondRow
			frame.y = frame.y + frame.h
			frame.x = frame.x + (index - firstRow -1 ) * frame.w
		end
